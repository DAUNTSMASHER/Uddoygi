// lib/features/factory/presentation/factory/all_work_orders_screen.dart
...
class AllWorkOrdersScreen extends StatefulWidget {
  const AllWorkOrdersScreen({Key? key}) : super(key: key);
...
class _AllWorkOrdersScreenState extends State<AllWorkOrdersScreen> {
  ...
  void _onNavTabSelected(FactoryNavTab tab) {
    if (tab == FactoryNavTab.all) return;
  ...
  Widget _buildOrdersList() {
    ...
    if (docs.isEmpty) {
      return const Center(child: Padding(padding: EdgeInsets.all(20), child: Text('No orders found.')));
    }
