...
class MarketingNoticeScreen extends StatefulWidget {
  const MarketingNoticeScreen({super.key});

  @override
  State<MarketingNoticeScreen> createState() => _MarketingNoticeScreenState();
}
...
class _MarketingNoticeScreenState extends State<MarketingNoticeScreen> {
  ...
  Future<void> _addComment(String noticeId, {String? parentCommentId}) async {
    ...
    await showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        title: Text(
          parentCommentId == null ? 'Add Comment' : 'Reply to Comment',
  ...
  Future<void> _deleteComment(String noticeId, String commentId, {String? parentCommentId}) async {
    if (parentCommentId == null) {
      await FirebaseFirestore.instance
          .collection('notices')
          .doc(noticeId)
  ...
  Future<void> _editComment(String noticeId, String commentId, String initialText, {String? parentCommentId}) async {
  ...
  Widget _buildReplies(String noticeId, String parentCommentId) {
      ...
      builder: (context, snapshot) {
        ...
        return Column(
          ...
          children: docs.map((doc) {
            ...
            final author = reply['authorName'] ?? reply['authorEmail'] ?? 'User';
            final text = reply['text'] ?? '';
            final fileUrl = reply['fileUrl'] ?? '';
            final fileName = reply['fileName'] ?? '';
            final ts = reply['timestamp'];
            ...
            return Padding(
              ...
              child: Container(
                ...
                child: ListTile(
                  leading: const Icon(Icons.reply, color: Colors.white54, size: 19),
                  title: Row(
                    children: [
                      Text(author, style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.white)),
                      const SizedBox(width: 8),
                      Text(DateFormat('MMM d, h:mm a').format(time), style: const TextStyle(fontSize: 11, color: Colors.white54)),
                    ],
                  ),
                  subtitle: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      ...
                      if (fileUrl.isNotEmpty)
                        TextButton.icon(
                          ...
                          label: Text(fileName, style: const TextStyle(color: Colors.white70)),
                          onPressed: () => _openFile(fileUrl),
                        ),
                    ],
                  ),
                  trailing: (reply['authorEmail'] == userEmail)
                    ...
                    onSelected: (value) {
                      if (value == 'edit') {
                        _editComment(noticeId, doc.id, text, parentCommentId: parentCommentId);
                      } else if (value == 'delete') {
                        _deleteComment(noticeId, doc.id, parentCommentId: parentCommentId);
                      }
                    },
              ...
              child: ListTile(
                ...
                subtitle: Column(
                  ...
                  children: [
                    ...
                    // Nested reply/comment feature (optional, Facebook style) can go here
                  ],
                ),
          ...
          child: Column(
            ...
            children: [
              ...
              if (isExpanded) ...[
                ...
                // Make comments area scrollable/flexible to avoid overflow
                ConstrainedBox(
                  constraints: const BoxConstraints(
                    maxHeight: 330, // Adjust as needed!
                  ),
                  child: _buildComments(id),
                ),
                const SizedBox(height: 10),
                Align(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        backgroundColor: Colors.indigo[900],
          title: const Text('Marketing Notices', style: TextStyle(color: Colors.white)),
